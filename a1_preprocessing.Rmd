...
knitr::opts_knit$set(root.dir = '/Users/annechristinevig/Documents/GitHub/CAHA')

library("tidyverse") #contains both stringr and dplyr 
rm(list=ls()) # clear global work space #ligesom at trykke på kosten
```
...
```{r rename_variables}
### Write your code here
df1_demo <- df1_demo %>% rename("SUBJ" = Child.ID)
df1_demo <- df1_demo %>% rename("VISIT" = Visit) 
```
...
```{r extract_digits}
#mutate create a new column, this new column is saved in the space of the original column 
df2_LU <- df2_LU %>%
...
```{r remove_points}
#Using mutate to add a column, overriding the previous column by '=', using \\. to identify '.', and '$' to specify end of a string/line 
