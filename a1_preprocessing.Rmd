...
Demo_participants$Visit <- str_extract(Demo_participants$Visit, "\\d+")
...
```{r merge_data}
merged_dataset <- bind_rows(Demo_participants, lu_data, Word_data)
...
# Filter the data to keep only the first visit for each child
first_visit_data <- merged_dataset %>%
  group_by(ID) %>%
  arrange(Visit) %>% 
  slice(1) %>%
  ungroup()
...
file_path <- "~/CAHA/cleaned_data.csv"
