...
- [Demographic data of the participants(demo_train)](https://www.dropbox.com/scl/fi/i0ty63jrjx6amp9ty5pbw/demo_train.csv?rlkey=wmx86tb0z12st8ec04dhvgmrr&dl=0)
- [Length of utterance data(LU_train)](https://www.dropbox.com/scl/fi/bk8vi5ra8cqkll0b4rqb1/LU_train.csv?rlkey=4giuq1cvk5jmn9kuwtuf0t2qh&dl=0)
- [Word data(token_train)](https://www.dropbox.com/scl/fi/h4dop19ctdubhokbfsn2q/token_train.csv?rlkey=wffzcf1211bglrx8239g3e0h8&dl=0)
...
```{r remove_points}
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Adam", "Adam.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "AS2", "AS2.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Albert", "Albert.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Alfie", "Alfie.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Allison", "Allison.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Annie", "Annie.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Audrey", "Audrey.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Charles", "Charles.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Dirk", "Dirk.")

demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Eduardo", "Eduardo.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Frankie", "Frankie.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Gabriel", "Gabriel.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "J.GII", "J.GII.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Jack", "Jack.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Jason", "Jason.")

demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Johan", "Johan.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Jerry", "Jerry.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "John", "John.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Judson", "Judson.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Kara", "Kara.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Kevin", "Kevin.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Lester", "Lester.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Luis", "Luis.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Marius", "Marius.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Milo", "Milo.")
demo_train$SUBJ <- str_replace(demo_train$SUBJ, "N.T.", "N.T")
demo_train$SUBJ <- str_replace(demo_train$SUBJ, "N.T", "N.T.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Omar", "Omar.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Rory", "Rory.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Ryder", "Ryder.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Tim", "Tim.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Tina", "Tina.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Todd", "Todd.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Vick", "Vick.")
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "Witt", "Witt.")

# Removing all participants only availaible in demo_train. (should probably do not use this.  though... Need to find a way to merge dataframes with different amount of observations)
# demo_train <- demo_train %>% filter(SUBJ != "A.A." & SUBJ != "Audrey." & SUBJ != "Gabriel." & SUBJ != "N.T." & SUBJ != "N.T.." & SUBJ != "P.D.")
...
#demo_train$SUBJ <- as.factor(demo_train$SUBJ) 
#LU_train$SUBJ <- as.factor(LU_train$SUBJ)

# demo_train <- demo_train %>% filter(SUBJ == LU_train$SUBJ & Visit == LU_train$Visit)


# demo_train <- demo_train %>% filter(demo_train$SUBJ == LU_train$SUBJ & demo_train$Visit == LU_train$Visit)
```
...
demo_train <- demo_train %>% select(SUBJ, Visit, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)


LU_train <- LU_train %>% select(SUBJ, Visit, MOT_MLU, CHI_MLU)

token_train <- token_train %>% select(SUBJ, Visit, types_MOT, types_CHI, tokens_MOT, tokens_CHI)

# Renaming variables
demo_train <- rename(demo_train, nonVerbalIQ = MullenRaw, VerbalIQ = ExpressiveLangRaw)
...
# Removing subjects only found in demo_train

merge_df <- merge(LU_train, demo_train, by = c("SUBJ", "Visit"), all = T)

merge_df <- merge(merge_df, token_train, by = c("SUBJ", "Visit"), all = T)

rm(demo_train, LU_train, token_train)
...
In order for our models to be useful, we want to minimize the need to actually test children as they develop. In other words, we would like to be able to predict the children's linguistic development after only having tested them once. Therefore we need to make sure that our *ADOS*, *MullenRaw(NonverbalIQ)*, *ExpressiveLangRaw(VerbalIQ)* and *Socialization* variables are reporting (for all visits) only the scores from visit 1.
...
### Deleting non useful dataframes

# * create a new data set having only first visits as rows as well as *child.ID* and the 4 relevant clinical variables as columns
df <- merge_df %>% filter(Visit == 1) %>% select(SUBJ, Visit, ADOS1 = ADOS, nonVerbalIQ1 = nonVerbalIQ, VerbalIQ1 = VerbalIQ, Socialization1 = Socialization)
# rename the clinical variables in a way that clearly indicates they relate to the first visit (e.g. *ADOS* to *ADOS1*) 
# Check
# merge the new data set with the old
merge_df <- merge(merge_df, df, by = c("SUBJ", "Visit"), all = T)
...
```{r revert_encoding}
# In the gender collumn chagne 1 and 2 to F and M respectively.
merge_df <- merge_df %>%
...
```{r}
# Define your DataFrame (merge_df)

# Specify the file path with double backslashes
file_path <- "C:\\Users\\cvbar\\OneDrive\\Skrivebord\\Methods 3\\Christian Branch\\CAHA\\data.csv"

# Attempt to write the CSV file
tryCatch({
  write.csv(merge_df, file_path, row.names = FALSE)
  cat("CSV file successfully written.\n")
}, error = function(e) {
  cat("Error:", e$message, "\n")
})
