...
#changing child.ID to SUBJ and visit to VISIT

names(demo_train)[names(demo_train) == 'Child.ID'] <- 'SUBJ'
names(demo_train)[names(demo_train) == 'Visit'] <- 'VISIT'


#make letters small, visit, maybe ethnicity

token_train$VISIT <- tolower(token_train$VISIT)
LU_train$VISIT <- tolower(LU_train$VISIT)


#X to visitX, done better in the chunk below

#VISIT <- as.character(demo_train$VISIT)

#demo_train <- demo_train %>% 
   # mutate(VISIT = paste("visit", VISIT, sep = ""))


#Remove punctuation behind, also done better below

#LU_train$SUBJ <- gsub(pattern = "[,.]$", replacement = "", LU_train$SUBJ)
#LU_train$VISIT <- gsub(pattern = "[,.]$", replacement = "",LU_train$VISIT)

#token_train$VISIT <- gsub(pattern = "[,.]$", replacement = "", token_train$VISIT)

#demo_train$SUBJ <- gsub(pattern = "[,.]$", replacement = "", demo_train$SUBJ)
...
#remove . at end of subj
demo_train$SUBJ <- str_replace_all(demo_train$SUBJ, "[,.]$", replacement = "")
LU_train$SUBJ <- str_replace_all(LU_train$SUBJ, "[,.]$", replacement = "")
token_train$SUBJ <- str_replace_all(token_train$SUBJ, "[,.]$", replacement = "")

#Then merge them together by the two shared columns
...
```{r}
df <- merge_df %>% 
    select(SUBJ, VISIT, Diagnosis, Ethnicity, Gender, Age, ADOS, MullenRaw, ExpressiveLangRaw, Socialization, MOT_MLU, CHI_MLU, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
...
df_visit1 <- df %>% 
    filter(VISIT == "1") %>% 
    select(SUBJ, ADOS, MullenRaw, ExpressiveLangRaw, Socialization)

#renaming

names(df_visit1)[names(df_visit1) == 'ADOS'] <- 'ADOS1'
names(df_visit1)[names(df_visit1) == 'MullenRaw'] <- 'MullenRaw1'
names(df_visit1)[names(df_visit1) == 'ExpressiveLangRaw'] <- 'ExpressivelangRaw1'
names(df_visit1)[names(df_visit1) == 'Socialization'] <- 'Socialization1'


#merge df and df_visit2
df_visit1_merge <- merge(df, df_visit1, by = c("SUBJ"), all = T)
...
df_visit1_merge$Gender <- str_replace_all(df_visit1_merge$Gender, "[1]", replacement = "F")
df_visit1_merge$Gender <- str_replace_all(df_visit1_merge$Gender, "[2]", replacement = "M")

#A = ASD, B = TD
df_visit1_merge$Diagnosis <- str_replace_all(df_visit1_merge$Diagnosis, "[A]", replacement = "ASD")
df_visit1_merge$Diagnosis <- str_replace_all(df_visit1_merge$Diagnosis, "[B]", replacement = "TD")

cleaned_df <- df_visit1_merge
...
```{r save_changes}
### Write your code here
#save(cleaned_df, file = "cleaned_ASD.csv")

write.csv(cleaned_df, "C:/Users/anelo/OneDrive/Documents/GitHub/Methods 3/CAHA\\cleaned_ASD.csv", row.names = F)
